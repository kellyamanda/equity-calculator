...
equity = st.sidebar.number_input(
    "Stock options granted, vested over 4 years", value=1000
)

st.sidebar.write("**Strike price**")
strike = st.sidebar.number_input(
    "Price at which you can purchase options ($)", value=0.01
)

st.sidebar.write("**Number of fully diluted shares**")
totshares = st.sidebar.number_input(
    "Existing shares at the time of the offer", value=1000000
)
...
with a:
    ...
    valuation1 = st.select_slider(
        "The value at which the company will exit ($M)",
        options=[
            0,
            10,
            25,
...
Use the following form to weight how likely you think each situation is.
"""
)
with st.form("key"):
    outcome_one_prob = st.slider("Outcome 1 Probability", 0.05, 1.0, 0.05)
    outcome_two_prob = st.slider("Outcome 2 Probability", 0.05, 1.0, 0.05)
    outcome_three_prob = st.slider("Outcome 3 Probability", 0.05, 1.0, 0.05)
    normalize = st.checkbox("Normalize Probabilities?", value=1)
    st.form_submit_button()
if normalize:
    sum_probs = outcome_one_prob + outcome_two_prob + outcome_three_prob
    outcome_one_prob = outcome_one_prob / sum_probs
    outcome_two_prob = outcome_two_prob / sum_probs
    outcome_three_prob = outcome_three_prob / sum_probs
weighted_outcome = (
    outcome1 * outcome_one_prob
    + outcome2 * outcome_two_prob
    + outcome3 * outcome_three_prob
)
